...
class GameMaster:
    ...
    def __init__(self, filename: str) -> None:
        ...
        GameMaster.hidden_map[GameMaster.__seeker.getPosition().x][GameMaster.__seeker.getPosition().y] = 0
        for hider in GameMaster.__hiders:
            GameMaster.hidden_map[hider.getPosition().x][hider.getPosition().y] = 0

    def is_game_over(self) -> bool:
        for hider in GameMaster.__hiders:
            if not hider.isFound(): return False
        return True 
    ...
    def seekerGetSurrounding() -> list[position]:
        ...
        for hider in GameMaster.__hiders:
            ...
            if not (abs(GameMaster.__seeker.getPosition().x - hider.getPosition().x) <= GameMaster.__seekerObserveRange 
            and abs(GameMaster.__seeker.getPosition().y - hider.getPosition().y) <= GameMaster.__seekerObserveRange):
                continue
            isObserable = True
            for pos in bresenham(GameMaster.__seeker.getPosition(), hider.getPosition()):
                if GameMaster.__map[pos.x][pos.y] == 1:
                    isObserable = False
                    break
            if isObserable: InSight.append(hider.getPosition())
        return InSight
        ...
        while True:
            ...
            while True:
                ...
                if self.is_game_over():
                    print('Game Over')
                    break
                GameMaster.__map[GameMaster.__seeker.getPosition().x][GameMaster.__seeker.getPosition().y] = 0
                GameMaster.__seeker.move()
                GameMaster.__map[GameMaster.__seeker.getPosition().x][GameMaster.__seeker.getPosition().y] = 3
                for hider in GameMaster.__hiders:
